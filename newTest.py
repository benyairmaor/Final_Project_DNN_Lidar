...
def prepare_dataset(voxel_size, source_path, target_path):
    ...
    trans_init = np.asarray([[-0.5754197841861329, 0.817372954385317, -0.028169583003715, 11.778369303008173],
                             [-0.7611987839242382, -0.5478349625282469, -0.34706377682485917, 14.264281414042465],
                             [-0.2991128270727379, -0.17826471123330384, 0.9374183747982869, 1.6731349336747363],
                             [0., 0., 0., 1.]])
...
def execute_global_registration(source_down, target_down, source_fpfh, target_fpfh, voxel_size):
    ...
    draw_registration_result(source_down, target_down, result.transformation, "RANSAC result")
    return result
...
def refine_registration(source, target, result_ransac):
    ...
    draw_registration_result(source, target, result.transformation, "ICP result")
    return result
...
if __name__ == '__main__':
    ...
    source, target, source_down, target_down, source_fpfh, target_fpfh = prepare_dataset(voxel_size, source_path, target_path)

    # Execute global registration by RANSAC and FPFH , print the result and the correspondence point set .
    result_ransac = execute_global_registration(source_down, target_down, source_fpfh, target_fpfh, voxel_size)
    print(result_ransac)
    print("Ransac correspondence_set: ", np.asarray(result_ransac.correspondence_set), "\n")
    ...
    print(result_icp)
    print("Icp correspondence_set: ", np.asarray(result_icp.correspondence_set), "\n")
