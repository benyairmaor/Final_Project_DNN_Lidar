...
def preprocess_point_cloud(source, target, voxel_size):
    ...
    pcd_fpfh_target = o3d.pipelines.registration.compute_fpfh_feature(
        target, o3d.geometry.KDTreeSearchParamHybrid(radius=radius_feature, max_nn=500))
...
########################################### Preprocessing (Front End) #########################################
    ...
    scoreMatrix, source_voxelCorrIdx, target_voxelCorrIdx = findCorrZeroOne(
        source_down_arr, target_down_arr, 0.1001)
    ...
    if VISUALIZATION:
        source_key_corr_arr = np.zeros((len(source_voxelCorrIdx), 3))
        target_key_corr_arr = np.zeros((len(source_voxelCorrIdx), 3))
    ...
    fpfhSourceTargetConcatenate = np.concatenate(
        (source_fpfh_arr, target_fpfh_arr), axis=0)
