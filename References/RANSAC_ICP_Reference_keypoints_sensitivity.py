...
if __name__ == '__main__':
    ...
    gamma_21s = np.array([0.07, 0.12, 0.18, 0.27, 0.4, 0.5])
    gamma_32s = np.array([0.07, 0.12, 0.18, 0.27, 0.4, 0.5])
    scores_fitness = np.ones(
        (2, len(directories), len(gamma_21s), len(gamma_32s))) * -1
    ...
    for directory in directories:
        ...
        for j in range(2):
            for gamma_21 in range(len(gamma_21s)):
                for gamma_32 in range(len(gamma_32s)):
                    if j == 0:
                    ...
                    source, target, source_down, target_down, source_down_c, target_down_c, source_fpfh, target_fpfh, M_result, listSource, listTarget = UR.prepare_dataset(
                        voxel_size, source_path, target_path, translation_M[i], "keypoints", VISUALIZATION, gamma_21=gamma_21, gamma_32=gamma_32)
    ...
    for y in range(scores_fitness.shape[1]):
        for x in range(scores_fitness.shape[0]):
            plt.plot(gamma_21s, scores_fitness[x][y], color='green')
            plt.plot(gamma_32s, scores_fitness[x][y], color='red')
            ...
            plt.plot(gamma_21s, scores_overlap[x][y], color='green')
            plt.plot(gamma_32s, scores_overlap[x][y], color='red')
            ...
            plt.plot(
                gamma_21s, scores_matrix_distance_rotation[x][y], color='green')
            plt.plot(
                gamma_32s, scores_matrix_distance_rotation[x][y], color='red')
            plt.xlabel('gamma_21/gamma_32 size')
            ...
            plt.plot(
                gamma_21s, scores_matrix_distance_translation[x][y], color='green')
            plt.plot(
                gamma_32s, scores_matrix_distance_translation[x][y], color='red')
            plt.xlabel('gamma_21/gamma_32 size')
